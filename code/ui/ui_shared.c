...
#ifndef JK2_CGAME
		|| (item->text && item->text[0] == '@' && item->asset != ui_s_language.modificationCount) // string package language changed
#endif
	)
	{
					...
					for (j = 0; j < listPtr->numColumns; j++)
					{
						...
						*/
						...
						if (text)
						{
							...
							DC->drawText(x + 4 + listPtr->columnInfo[j].pos, y + textyOffset + item->textaligny, item->textscale, item->window.foreColor, text, 0, listPtr->columnInfo[j].maxChars, item->textStyle, item->iMenuFont);
						}
...
qboolean ItemParse_cvarStrList(itemDef_t *item, int handle)
{
	...
	if (!Q_stricmp(token.string, "feeder") && item->special == FEEDER_PLAYER_SPECIES)
	{
		...
		for (; multiPtr->count < uiInfo.playerSpeciesCount; multiPtr->count++)
		{
			...
			Q_strupr(name);
			trap_SP_GetStringTextString(&name[1], buffer, sizeof(buffer));
