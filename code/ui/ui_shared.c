...
#ifndef JK2_CGAME // Defined in ui_main.c, not in the namespace
...
qboolean PC_String_Parse(int handle, const char **out)
{
	...
	if (!Menu_IsJKA(NULL) && token.string[0] == '@') // Is it a localized text?
	{
		char *temp;
	...
	case A_MOUSE2:
		if (item)
		{
			if (item->type == ITEM_TYPE_TEXT)
			{
				...
				{
					rect = Item_CorrectedTextRect(item);
				}

				if (Rect_ContainsPoint(rect, DC->cursorx, DC->cursory))
				{
					Item_Action(item);
				}
			}
			...
			*/
			...
			// JLFACCEPT MPMOVED
			else if (Menu_IsJKA(menu) && (item->type == ITEM_TYPE_MULTI || item->type == ITEM_TYPE_YESNO || item->type == ITEM_TYPE_SLIDER || item->type == ITEM_TYPE_INTSLIDER))
			{
...
#ifndef JK2_CGAME
		|| (Item_IsJKA(item) && item->text && item->text[0] == '@' && item->asset != ui_s_language.modificationCount) // string package language changed
#endif
	)
	{
...
#ifndef JK2_CGAME
		if (Item_IsJKA(item) && item->text && item->text[0] == '@') // string package
		{															// mark language
			item->asset = ui_s_language.modificationCount;
		}
#endif
	}
}
...
void Item_Text_AutoWrapped_Paint(itemDef_t *item)
{
	...
	if (Item_IsJKA(item) && *textPtr == '@') // string reference
	{
...
void Item_TextField_Paint(itemDef_t *item)
{
	...
	{
		DC->getCVarString(item->cvar, buff, sizeof(buff));
		if (Item_IsJKA(item) && buff[0] == '@') // string reference
		{
...
void Item_Multi_Paint(itemDef_t *item)
{
	...
	if (Item_IsJKA(item) && *text == '@') // string reference
	{
		trap_SP_GetStringTextString(&text[1], temp, sizeof(temp));
		text = temp;
	}
	// Is is specifying a cvar to get the item name from?
	else if (Item_IsJKA(item) && *text == '*')
	{
...
void Item_ListBox_Paint(itemDef_t *item)
{
	...
	if (listPtr->elementStyle == LISTBOX_IMAGE)
	{ // The listbox type for images uses borderColor to outline elements
		Vector4Copy(item->window.borderColor, hoverColor);
		hoverColor[3] *= 0.3f;
	}
	else
	{ // Other types of listbox have a specific outline color
	...
	if (item->window.flags & WINDOW_HORIZONTAL)
	{
		...
		if (!listPtr->scrollhidden)
		{
			...
			if (Item_ListBox_MaxScroll(item) <= 0 && listPtr->startPos > 0)
			{
				listPtr->startPos = 0;
			}
		}
	...
	else
	{
		...
		else
		{
			...
			for (; i < count; i++)
			{
					...
					for (j = 0; j < listPtr->numColumns; j++)
					{
						...
						if (optionalImage4 >= 0 && menuInJK2MV)
						{
							if (gameVersion == VERSION_1_04 || protocol == PROTOCOL16)
							{
								...
								DC->fillRect(x + 2, y + listPtr->elementHeight + 2, item->window.rect.w - SCROLLBAR_SIZE - 4, listPtr->elementHeight, color);
							}
						...
						if (optionalImage4 >= 0 && menuInJK2MV)
						{
							DC->drawHandlePic(540, y + listPtr->elementHeight + 2, listPtr->elementHeight, listPtr->elementHeight, optionalImage4);
						}
						if (optionalImage3 >= 0)
						{
				...
				{
					DC->fillRect(x + 2, y + listPtr->elementHeight + 2, item->window.rect.w - SCROLLBAR_SIZE - 4, listPtr->elementHeight, hoverColor);
				}
...
void Item_Paint(itemDef_t *item)
{
	...
	if (item->window.flags & WINDOW_MOUSEOVER)
	{
		if (item->descText && !Display_KeyBindPending())
		{
			...
			if (!Item_IsJKA(item) && !Rect_ContainsPoint(&item->window.rect, DC->cursorx, DC->cursory))
...
menuDef_t *Menus_ActivateByName(const char *p)
{
	...
	for (i = 0; i < menuCount; i++)
	{
		...
		{
			// Hide all other menus and remove focus
			Menus[i].window.flags &= ~(WINDOW_HASFOCUS | WINDOW_VISIBLE);
		}
	}
	Display_CloseCinematics();
			...
			if (Rect_ContainsPoint(&menu->items[i]->window.rect, x, y))
			{
				if (pass == 1)
				{
					...
					if (overItem->type == ITEM_TYPE_TEXT && overItem->text)
					{
						...
						if (Menu_IsJKA(menu))
						{
							r = &overItem->window.rect;
						}
						else
						{
							r = Item_CorrectedTextRect(overItem);
						}
						if (!Rect_ContainsPoint(r, x, y))
						{
							continue;
...
static void Item_TextScroll_BuildLines(itemDef_t *item)
{
	...
	if (jk2version != VERSION_1_02)
	{
		...
		if (Item_IsJKA(item) && *psText == '@') // string reference
		{
		...
		while (*psCurrentTextReadPos && (scrollPtr->iLineCount < MAX_TEXTSCROLL_LINES))
		{
			...
			while (*psCurrentTextReadPos)
			{
				...
				else if (DC->textWidth(sLineForDisplay, item->textscale, item->iMenuFont) >= iBoxWidth)
				{
	...
	else
	{
		...
		if (Item_IsJKA(item) && *lineStart == '@') // string reference
		{
			trap_SP_GetStringTextString(&lineStart[1], text, sizeof(text));
			lineStart = text;
		}
...
// I need this global variable when menu->isJKA is not available
qboolean currentMenuIsJKA = qfalse;

void Menu_SetJKA(qboolean isJKA)
{
	currentMenuIsJKA = isJKA;
}

qboolean Menu_IsJKA(menuDef_t *menu)
{
	return (menu != NULL) ? menu->isJKA : currentMenuIsJKA;
}

qboolean Item_IsJKA(itemDef_t *item)
{
	menuDef_t *menu = NULL;
	if (item != NULL)
	{
