...
typedef struct {
  ...
  const char *(*feederItemText)(float feederID, int index, int column, qhandle_t *handle1, qhandle_t *handle2, qhandle_t *handle3, qhandle_t *handle4, qhandle_t *handle5, qhandle_t *handle6);
	...
	Q_PTR_NORETURN void (*Error)(errorParm_t level, PRINTF_FORMAT_STRING const char *error, ...) __attribute__ ((format (printf, 2, 3)));
	void (*Print)(PRINTF_FORMAT_STRING const char *msg, ...) __attribute__ ((format (printf, 1, 2)));
