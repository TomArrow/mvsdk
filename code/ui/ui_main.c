...
void _UI_DrawRect(float x, float y, float width, float height, float size, const float *color)
{
	...
	float sx = x * uiInfo.screenXFactorInv;
	float sy = y * uiInfo.screenYFactorInv;
	float sw = width * uiInfo.screenXFactorInv;
	float sh = height * uiInfo.screenYFactorInv;
	_UI_DrawTopBottom(sx, sy, sw, sh, size * uiInfo.screenXFactorInv);
	_UI_DrawSides(sx, sy, sw, sh, size * uiInfo.screenXFactorInv);
...
static void Text_Paint(float x, float y, float scale, const vec_t *color, const char *text, float adjust, int limit, int style, int iMenuFont)
{
	...
	float sx = x * uiInfo.screenXFactorInv;
	float sy = y * uiInfo.screenYFactorInv;
	trap_R_Font_DrawString(sx,					  // int ox
						   sy,					  // int oy
						   text,				  // const char *text
						   color,				  // paletteRGBA_c c
						   iStyleOR | iFontIndex, // const int iFontHandle
...
static const char *UI_GetGameTypeName(int gtEnum)
{
	...
	{
	case GT_FFA:
		return UI_GetStripEdString("MENUS1", "FREE_FOR_ALL"); // "Free For All"
	case GT_HOLOCRON:
		return UI_GetStripEdString("MENUS1", "HOLOCRON_FFA"); // "Holocron FFA"
	case GT_JEDIMASTER:
		return UI_GetStripEdString("MENUS3", "NOFORCE_JEDI_0"); // "Jedi Master"
	case GT_TOURNAMENT:
		return UI_GetStripEdString("MENUS1", "DUEL"); // "Duel"
	case GT_SINGLE_PLAYER:
		return UI_GetStripEdString("MENUS1", "SAGA"); // "N/A"
	case GT_TEAM:
		return UI_GetStripEdString("MENUS1", "TEAM_FFA"); // "Team FFA"
	case GT_SAGA:
		return UI_GetStripEdString("MENUS1", "SAGA"); // "N/A"
	case GT_CTF:
		return UI_GetStripEdString("MENUS1", "CAPTURE_THE_FLAG"); // "Capture the Flag"
	case GT_CTY:
		return UI_GetStripEdString("MENUS1", "CAPTURE_THE_YSALIMARI"); // "Capture the Ysalamiri"
	default:
		return UI_GetStripEdString("MENUS1", "SAGA"); // "N/A"
	}
}
...
static void UI_DrawSkinColor(rectDef_t *rect, float scale, vec4_t color, int textStyle, int val, int min, int max, int iMenuFont)
{
	...
	{
	case SKINCOLOR_RED:
		trap_SP_GetStringTextString("MENUS0_TEAM_RED", s, sizeof(s));
		break;
	case SKINCOLOR_BLUE:
		trap_SP_GetStringTextString("MENUS0_TEAM_BLUE", s, sizeof(s));
		break;
	case SKINCOLOR_OTHER:
		trap_SP_GetStringTextString("MENUS1_OTHER", s, sizeof(s));
		break;
	default:
		trap_SP_GetStringTextString("MENUS0_DEFAULT", s, sizeof(s));
		break;
	}
...
static void UI_DrawPlayerModel(rectDef_t *rect) {
  ...
  char team[256];
	...
	vec3_t	moveangles;
	  ...
	  Q_strncpyz(model, UI_Cvar_VariableString("model"));
		Q_strncpyz(head, UI_Cvar_VariableString("headmodel"));
		if (!q3Model) {
			q3Model = qtrue;
			updateModel = qtrue;
		}
		...
		Q_strncpyz(team, UI_Cvar_VariableString("ui_teamName"));
		Q_strncpyz(model, UI_Cvar_VariableString("team_model"));
		Q_strncpyz(head, UI_Cvar_VariableString("team_headmodel"));
		if (q3Model) {
			q3Model = qfalse;
			updateModel = qtrue;
		}
...
static int UI_OwnerDrawWidth(int ownerDraw, float scale)
{
	...
	case UI_SKIN_COLOR:
		...
		{
		case SKINCOLOR_RED:
			s = (char *)UI_GetStripEdString("MENUS0", "TEAM_RED");
			break;
		case SKINCOLOR_BLUE:
			s = (char *)UI_GetStripEdString("MENUS0", "TEAM_BLUE");
			break;
		case SKINCOLOR_OTHER:
			s = (char *)UI_GetStripEdString("MENUS1", "OTHER");
			break;
		default:
			s = (char *)UI_GetStripEdString("MENUS0", "DEFAULT");
			break;
		}
		break;
	case UI_FORCE_SIDE:
	...
	case UI_BLUETEAMNAME:
		...
		if (i >= 0 && i < uiInfo.teamCount)
		{
			s = va("%s: %s", (char *)UI_GetStripEdString("MENUS0", "TEAM_BLUE"), uiInfo.teamList[i].teamName);
		}
		break;
	case UI_REDTEAMNAME:
		...
		if (i >= 0 && i < uiInfo.teamCount)
		{
			s = va("%s: %s", (char *)UI_GetStripEdString("MENUS0", "TEAM_RED"), uiInfo.teamList[i].teamName);
		}
		break;
	case UI_BLUETEAM1:
	case UI_BLUETEAM2:
...
{
	Text_Paint(rect->x, rect->y, scale, color, (uiInfo.redBlue == 0) ? UI_GetStripEdString("MENUS0", "TEAM_RED") : UI_GetStripEdString("MENUS0", "TEAM_BLUE"), 0, 0, textStyle, iMenuFont);
}
...
static void UI_BuildPlayerList()
{
	...
	for (n = 0; n < count; n++)
	{
		...
		if (info[0])
		{
			...
			Q_CleanStr(uiInfo.playerNames[uiInfo.playerCount], qtrue, qfalse);
			...
			if (team2 == team && n != uiInfo.playerNumber)
			{
				...
				Q_CleanStr(uiInfo.teamNames[uiInfo.myTeamCount], qtrue, qfalse);
...
static void UI_LoadDemos()
{
	...
	for (compressed = 0; compressed <= 1; compressed++)
	{
		...
		{
#ifndef JK2MV_MENU
			if ((protocol == PROTOCOL15 && jk2startversion != VERSION_1_02 && jk2startversion != VERSION_1_03) || (protocol == PROTOCOL16 && jk2startversion != VERSION_1_04) || jk2startversion == VERSION_UNDEF)
			{
				continue;
			}
#endif
...
static void UI_SetBotButton(void)
{
	...
	if ((gameType == GT_SAGA) || (server == 0)) // If it's not siege, don't worry about it
	{
		menu = Menu_GetFocused(); // Get current menu (either video or ingame video, I would assume)
...
static void UI_RunMenuScript(const char **args)
{
	...
	if (String_Parse(args, &name))
	{
		...
		else if (Q_stricmp(name, "weaponDisable") == 0)
		{
			...
			if (uiInfo.gameTypes[ui_netGameType.integer].gtEnum == GT_TOURNAMENT)
			{
...
void UI_ClampMaxPlayers(void)
{
	...
	// min checks
	//
	if (uiInfo.gameTypes[ui_netGameType.integer].gtEnum == GT_TOURNAMENT) // DUEL
	{
...
static q3Head_t *UI_GetHeadByName(const char *name)
{
	...
	{
		if (Q_stricmp(name, uiInfo.q3Heads[i].name) == 0)
		{
			return &uiInfo.q3Heads[i];
		}
	}
...
static void UI_InsertHeadRaw(q3Head_t *head)
{
	...
	if (uiInfo.q3HeadCount >= MAX_PLAYER_SKINS)
	{
		Com_Printf(S_COLOR_YELLOW "WARNING: MAX_PLAYER_SKINS exceeded (%d)\n", MAX_PLAYER_SKINS);
		return;
	}

	current = &uiInfo.q3Heads[uiInfo.q3HeadCount];
	*current = *head;
	uiInfo.q3HeadCount++;
}

static void UI_InsertHead(const char *model, const char *skin)
{
	static q3Head_t newHead;
	...
	Com_sprintf(headName, sizeof(headName), "%s/%s", model, skin);

	// Prevent duplicates
	if (UI_GetHeadByName(headName))
	{
		return;
	}

	Q_strncpyz(newHead.name, headName, sizeof(newHead.name));
	Com_sprintf(headName, sizeof(headName), "models/players/%s/icon_%s", model, skin);
	newHead.icon = trap_R_RegisterShaderNoMip(headName);
...
static void UI_BuildServerDisplayList(int force)
{
	...
	if (len == 0)
	{
		Q_strncpyz(uiInfo.serverStatus.motd, "Welcome to JK2MP!", sizeof(uiInfo.serverStatus.motd));
		len = (int)strlen(uiInfo.serverStatus.motd);
	}
...
static void UI_BuildFindPlayerList(qboolean force)
{
	...
	else
	{
		...
		Q_CleanStr(uiInfo.findPlayerName, (qboolean)(jk2startversion == VERSION_1_02), qfalse);
		...
		if (uiInfo.pendingServerStatus.server[i].valid)
		{
			...
			if (UI_GetServerStatusInfo(uiInfo.pendingServerStatus.server[i].adrstr, &info))
			{
				...
				for (j = 0; j < info.numLines; j++)
				{
					...
					Q_CleanStr(name, (qboolean)(jk2startversion == VERSION_1_02), qfalse);
...
const char *UI_GetModelWithSkin(const char *model)
{
	...
	if (serverGameType >= GT_TEAM)
	{
		...
		if (currentColor == uiSkinColor || (myTeam != TEAM_RED && myTeam != TEAM_BLUE) || strchr(model, '|') != NULL)
			return model;
		ptr = strchr(model, '/');

		if (!ptr)
			Q_strncpyz(modelWithSkin, model, sizeof(modelWithSkin));
		else if ((unsigned long)(ptr - model + 1) <= sizeof(modelWithSkin))
			Q_strncpyz(modelWithSkin, model, ptr - model + 1);
		else
			return model; // Error
...
int UI_HeadIndexForModel(const char *model)
{
	...
	for (i = 0; i < uiInfo.q3HeadCount; i++)
	{
		...
		if (UI_HeadBelongsToCurrentTeamColor(head))
		{
			if (Q_stricmp(head->name, model) == 0)
			{
				return c;
			}
	...
	else if (feederID == FEEDER_SERVERS)
	{
		if (index >= 0 && index < uiInfo.serverStatus.numDisplayServers)
		{
			...
			case SORT_GAME:
				...
				{
					Q_strncpyz(needPass, teamArenaGameTypes[game], sizeof(needPass));
				}
				else
				{
					if (ping <= 0)
...
static qhandle_t UI_FeederItemImage(float feederID, int index)
{
			...
			if (selModel == -1 && playerModel)
			{
				if (!Q_stricmpn("secret_quigon/", playerModel, strlen("secret_quigon/")))
				{
					if (!Q_stricmp("secret_quigon/red", playerModel))
					{
						...
						if (noIconHead.name[0] == '\0')
						{
							noIconHead.icon = trap_R_RegisterShaderNoMip("models/players/secret_quigon/icon_red");
							Q_strncpyz(noIconHead.name, playerModel, sizeof(noIconHead.name));
	...
	else if (feederID == FEEDER_PLAYER_SKIN_HEAD)
	{
		if (index >= 0 && index < uiInfo.playerSpecies[uiInfo.playerSpeciesIndex].SkinHeadCount)
		{
			return uiInfo.playerSpecies[uiInfo.playerSpeciesIndex].SkinHead[index].icon;
		}
	}
	else if (feederID == FEEDER_PLAYER_SKIN_TORSO)
	{
		if (index >= 0 && index < uiInfo.playerSpecies[uiInfo.playerSpeciesIndex].SkinTorsoCount)
		{
			return uiInfo.playerSpecies[uiInfo.playerSpeciesIndex].SkinTorso[index].icon;
		}
	}
	else if (feederID == FEEDER_PLAYER_SKIN_LEGS)
	{
		if (index >= 0 && index < uiInfo.playerSpecies[uiInfo.playerSpeciesIndex].SkinLegCount)
		{
			return uiInfo.playerSpecies[uiInfo.playerSpeciesIndex].SkinLeg[index].icon;
		}
	}
	else if (feederID == FEEDER_COLORCHOICES)
	{
		if (index >= 0 && index < uiInfo.playerSpecies[uiInfo.playerSpeciesIndex].ColorCount)
		{
			return uiInfo.playerSpecies[uiInfo.playerSpeciesIndex].Color[index].icon;
		}
	}
...
qboolean UI_FeederSelection(float feederFloat, int index, itemDef_t *item)
{
				...
				if (modelPtr)
				{
					...
					if (datapadMoveData[uiInfo.movesTitleIndex][index].anim)
					{
						...
						else if (datapadMoveData[uiInfo.movesTitleIndex][index].sound == MDS_SABER)
						{
							...
							int soundI = Q_irand(1, 6, qfalse, 0);
	...
	else if (feederID == FEEDER_PLAYER_SKIN_HEAD)
	{
		if (index >= 0 && index < uiInfo.playerSpecies[uiInfo.playerSpeciesIndex].SkinHeadCount)
		{
			trap_Cvar_Set("ui_char_skin_head", uiInfo.playerSpecies[uiInfo.playerSpeciesIndex].SkinHead[index].name);
		}
	}
	else if (feederID == FEEDER_PLAYER_SKIN_TORSO)
	{
		if (index >= 0 && index < uiInfo.playerSpecies[uiInfo.playerSpeciesIndex].SkinTorsoCount)
		{
			trap_Cvar_Set("ui_char_skin_torso", uiInfo.playerSpecies[uiInfo.playerSpeciesIndex].SkinTorso[index].name);
		}
	}
	else if (feederID == FEEDER_PLAYER_SKIN_LEGS)
	{
		if (index >= 0 && index < uiInfo.playerSpecies[uiInfo.playerSpeciesIndex].SkinLegCount)
		{
			trap_Cvar_Set("ui_char_skin_legs", uiInfo.playerSpecies[uiInfo.playerSpeciesIndex].SkinLeg[index].name);
		}
	}
	else if (feederID == FEEDER_PLAYER_SPECIES)
	{
...
static void UI_BuildQ3Model_List(void)
{
	...
	for (i = 0; i < numfiles; i++, fileptr += filelen + 1)
	{
		...
		*skin = 0;
		...
		if (skinLen == -1)
			skinLen = (int)strlen(skin);

		k = 0;
		while (k < skinLen && skin[k] && skin[k] != '_')
			k++;
		...
		UI_InsertHead(model, check);
	}
	if (fileBufSize)
		BG_TempFree(fileBufSize);
}
...
static qboolean UI_ParseColorData(char *buf, playerSpeciesInfo_t *species, char *file)
{
	...
	while (p)
	{
		...
		if (species->ColorCount >= MAX_PLAYER_SPECIE_COLORS)
		{
			Com_Printf(S_COLOR_YELLOW "WARNING: MAX_PLAYER_SPECIE_COLORS exceeded (%d)\n", MAX_PLAYER_SPECIE_COLORS);
			return species->ColorCount;
		}

		Q_strncpyz(species->Color[species->ColorCount].shader, token, MAX_QPATH);
		species->Color[species->ColorCount].icon = trap_R_RegisterShaderNoMip(token);
		...
		assert(!species->Color[species->ColorCount].actionText[0]);
		...
		while (token[0] != '}')
		{
			...
			assert(species->ColorCount < MAX_PLAYER_SPECIE_COLORS);
	...
	for (i = 0; i < numdirs; i++, dirptr += dirlen + 1)
	{
		...
		if (f)
		{
			...
			for (j = 0; j < numfiles; j++, fileptr += filelen + 1)
			{
				...
				Com_sprintf(iconName, sizeof(iconName), "models/players/%s/icon_%s", dirptr, skinname);
				iconHandle = trap_R_RegisterShaderNoMip(iconName);

				if (iconHandle != 0)
				{ // if it exists
					playerSpeciesInfo_t *specie;
					skinName_t *specieSkin;

					specie = &uiInfo.playerSpecies[uiInfo.playerSpeciesCount];
					if (Q_stricmpn(skinname, "head_", 5) == 0)
					{
						if (specie->SkinHeadCount >= MAX_PLAYER_SPECIE_HEADS)
						{
							Com_Printf(S_COLOR_YELLOW "WARNING: MAX_PLAYER_SPECIE_HEADS exceeded (%d)\n", MAX_PLAYER_SPECIE_HEADS);
							continue;
						}

						specieSkin = &specie->SkinHead[specie->SkinHeadCount];
						Q_strncpyz(specieSkin->name, skinname, SKIN_LENGTH);
						iSkinParts |= 1 << 0;
						specieSkin->icon = iconHandle;
						specie->SkinHeadCount++;
					}
					else if (Q_stricmpn(skinname, "torso_", 6) == 0)
					{
						if (specie->SkinTorsoCount >= MAX_PLAYER_SPECIE_TORSOS)
						{
							Com_Printf(S_COLOR_YELLOW "WARNING: MAX_PLAYER_SPECIE_TORSOS exceeded (%d)\n", MAX_PLAYER_SPECIE_TORSOS);
							continue;
						}

						specieSkin = &specie->SkinTorso[specie->SkinTorsoCount];
						Q_strncpyz(specieSkin->name, skinname, SKIN_LENGTH);
						iSkinParts |= 1 << 1;
						specieSkin->icon = iconHandle;
						specie->SkinTorsoCount++;
					}
					else if (Q_stricmpn(skinname, "lower_", 6) == 0)
					{
						if (specie->SkinLegCount >= MAX_PLAYER_SPECIE_LOWERS)
						{
							Com_Printf(S_COLOR_YELLOW "WARNING: MAX_PLAYER_SPECIE_LOWERS exceeded (%d)\n", MAX_PLAYER_SPECIE_LOWERS);
							continue;
						}

						specieSkin = &specie->SkinLeg[specie->SkinLegCount];
						Q_strncpyz(specieSkin->name, skinname, SKIN_LENGTH);
						iSkinParts |= 1 << 2;
						specieSkin->icon = iconHandle;
						specie->SkinLegCount++;
			...
			if (uiInfo.playerSpeciesCount >= MAX_PLAYER_SPECIES)
			{
				Com_Printf(S_COLOR_YELLOW "WARNING: MAX_PLAYER_SPECIES exceeded (%d)\n", MAX_PLAYER_SPECIES);
				return;
			}
		}
	}
...
void UI_DrawConnectScreen(qboolean overlay)
{
	...
	else
	{
		...
		Q_strncpyz(text, va(/*"Connecting to %s"*/ sStripEdTemp, cstate.servername), sizeof(text));
...
vmCvar_t ui_sp_language;
vmCvar_t ui_se_language;
...
static const cvarTable_t cvarTable[] = {
	...
	{&ui_spScores1, "g_spScores1", "", CVAR_ARCHIVE | CVAR_ROM | CVAR_INTERNAL},
	{&ui_spScores2, "g_spScores2", "", CVAR_ARCHIVE | CVAR_ROM | CVAR_INTERNAL},
	{&ui_spScores3, "g_spScores3", "", CVAR_ARCHIVE | CVAR_ROM | CVAR_INTERNAL},
	{&ui_spScores4, "g_spScores4", "", CVAR_ARCHIVE | CVAR_ROM | CVAR_INTERNAL},
	{&ui_spScores5, "g_spScores5", "", CVAR_ARCHIVE | CVAR_ROM | CVAR_INTERNAL},
	{&ui_spAwards, "g_spAwards", "", CVAR_ARCHIVE | CVAR_ROM | CVAR_INTERNAL},
	{&ui_spVideos, "g_spVideos", "", CVAR_ARCHIVE | CVAR_ROM | CVAR_INTERNAL},
	{&ui_spSkill, "g_spSkill", "2", CVAR_ARCHIVE | CVAR_INTERNAL},
	{&ui_spSelection, "ui_spSelection", "", CVAR_ROM | CVAR_INTERNAL},
	...
	{&ui_cdkeychecked, "ui_cdkeychecked", "0", CVAR_ROM | CVAR_INTERNAL},
	...
	{&ui_currentTier, "ui_currentTier", "0", CVAR_ARCHIVE | CVAR_INTERNAL},
	...
	{&ui_Q3Model, "ui_q3model", "0", CVAR_ARCHIVE | CVAR_INTERNAL},
	...
	{&ui_sp_language, "sp_language", "0", CVAR_ARCHIVE | CVAR_NORESTART},
	{&ui_se_language, "se_language", "english", CVAR_ARCHIVE | CVAR_NORESTART},
...
int spLanguageModificationCount = -1;
int seLanguageModificationCount = -1;
/*
=================
UI_RegisterCvars
=================
...
void UI_UpdateTextLanguageCvar(qboolean updateCvarFromJKA)
{
	char se_language_cvar[128];
	char sp_language_cvar[4];
	int languageIndex;
	int numLanguages;
	...
	{
		if (updateCvarFromJKA)
		{
			trap_Cvar_Set("se_language", languageName);
		}
		else
		{
			...
			if (languageIndex >= numLanguages)
			{
				languageIndex = 0;
			}
			trap_Cvar_SetValue("sp_language", (float)languageIndex);
		}
	}
