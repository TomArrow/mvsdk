...
clientkilled:
	...
	if ( attacker == cg.snap->ps.clientNum ) {
		...
		if (!(cg_singlePlayerActive.integer && cg_cameraOrbit.integer)) {
			CG_CenterPrint( s, cgDC.screenHeight * 0.30, BIGCHAR_WIDTH );
		} 
...
static void CG_UseItem( centity_t *cent ) {
	...
	if ( es->number == cg.snap->ps.clientNum ) {
		if ( !itemNum ) {
			CG_CenterPrint( "No item to use", cgDC.screenHeight * 0.30, BIGCHAR_WIDTH );
		} else {
		...
		DEBUGNAME("EV_ITEM_PICKUP");
		{
			...
			{ //a holocron most likely
				...
				if (es->number == cg.snap->ps.clientNum && showPowersName[index])
				{
					...
					CG_CenterPrint( va("%s %s\n", strText, CG_GetStripEdString("INGAME",showPowersName[index])), cgDC.screenHeight * 0.30, BIGCHAR_WIDTH );
				}
