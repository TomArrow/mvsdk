...
static void CG_DrawSimpleForcePower(const centity_t *cent)
{
	...
	if (cg_hudColors.integer)
	{ // JAPRO - Clientside - Gradient simple hud coloring
...
void DF_RaceTimer(void)
{
...

	{
		...
		if (time < cg.lastRaceTime && (!(cg.predictedPlayerState.stats[STAT_RUNFLAGS] & RFL_SEGMENTED) || time < 1000))
		{ // in segmented mode, don't reset these stats when we respos (shitty dumb workaround but ok for now, TODO better later)
			cg.startSpeed = 0;
			cg.displacement = 0;
			cg.maxSpeed = 0;
			cg.displacementSamples = 0;
		...
		if (cg_raceTimer.integer != 0)
		{
			...
			if (cg_raceTimer.integer > 1 && !cg_raceTimerNoSpeeds.integer)
			{
				if (cg.displacementSamples)
					Q_strcat(timerStr, sizeof(timerStr), va("Max: %i\nAvg: %i", (int)((float)cg.maxSpeed + 0.5f), cg.displacement / cg.displacementSamples));
...
void CG_DrawHUD(centity_t *cent)
{
	...
	if (cg_drawAntiLoopIndicator.integer /* && !cg.demoPlayback*/)
	{ // TODO make work for demos?
	...
	if (cg_strafeHelper.integer & SHELPER_CROSSHAIR)
	{
		...
		if (!cg.crosshairColor[0] && !cg.crosshairColor[1] && !cg.crosshairColor[2])
		{ // default to white
			hcolor[0] = 1.0f;
			hcolor[1] = 1.0f;
			hcolor[2] = 1.0f;
...
static float CG_DrawFPS(float y, float oldY, qboolean physical)
{
	...
	if (data->index > FPS_FRAMES)
	{
		...
		if (cg_drawFPSMisses.integer)
		{
			...
			if (!shortCount || guessedMsecShort == guessedMsec)
			{ // a technique we will be using on the server for fps detection. short average must match long one. to avoid misdetects on transitions. e.g. 32 samples and 8 short sequence samples
				if (data->indexMisses >= MISS_FRAMES)
					data->indexMisses = 0;
...
static void CG_DrawReward(void)
{
	...
	if (time <= ITEM_BLOB_TIME)
	{ // fade in
		iconSize *= time * (1.0 / ITEM_BLOB_TIME);
	}
	else if (time > 0 && REWARD_TIME - time <= ITEM_BLOB_TIME)
	{ // fade out
...
void JKMod_CG_DrawClock(void)
{
	...
	if (cg.snap->ps.pm_type == PM_SPECTATOR && cgs.gametype == GT_TOURNAMENT)
		y -= 97;
	...
	if (jkcvar_cg_drawClock.integer == 2)
		CG_Text_Paint(x + 47, y + 13.5f, 0.4f, colorTable[CT_HUD_GREEN], va("%s", systemTimeType), 0, 0, UI_SMALLFONT | UI_DROPSHADOW, FONT_SMALL);
	if ((cg.time >> 9) & 1)
...
static void CG_Draw2D(void)
{
	...
	*/
	...
	// V24 Enhanced Features - ESP Drawing
	if (cg_esp.integer)
	{
		CG_UpdateESP();
		CG_DrawESP();
...
static void CG_DrawBouncePowerMeter(void)
{
	...
	if (!cgs.isTommyTernal || !cg.predictedPlayerState.stats[STAT_RACEMODE] || cg.predictedPlayerState.stats[STAT_MOVEMENTSTYLE] != MV_BOUNCE)
		return;
...
static void CG_AntiLoopIndicator(void)
{
	...
	if ((!cgs.isTommyTernal || !cg.predictedPlayerState.stats[STAT_RACEMODE] /*|| !MovementStyleHasAntiLoop(cg.predictedPlayerState.stats[STAT_MOVEMENTSTYLE])*/ || !(cg.predictedPlayerState.stats[STAT_RUNFLAGS] & RFL_ANTILOOP) || cg.predictedPlayerState.duelTime) && cg_drawAntiLoopIndicator.integer <= 1)
		return;
...
static void CG_JumpHeight(centity_t *cent)
{
	...
	if (cg.predictedPlayerState.fd.forceJumpZStart && (cg.lastZSpeed > 0) && (velocity[2] <= 0))
...
static void DrawStrafeLine(vec3_t velocity, float diff, qboolean active, int moveDir)
{ // moveDir is 1-7 for wasd combinations, and 8 for the centerline in cpm style, 9 and 10 for backwards a/d lines
	...
	if (cg_strafeHelper.integer & SHELPER_OLDBARS && active && moveDir != 0)
	{ // Not sure how to deal with multiple lines for W only so just fuck it for now..
	...
	if (cg_strafeHelper.integer & SHELPER_SOUND && active && moveDir != 8)
