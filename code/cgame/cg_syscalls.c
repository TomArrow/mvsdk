...
void trap_CM_BoxTrace(trace_t *results, const vec3_t start, const vec3_t end,
					  ...
					  clipHandle_t model, int brushmask, qboolean customEpsilonTrace, float customEpsilon, int traceCustomFlags // coolapi extension. normally these extra values would be random/undefined anyway so we may as well only check engine side
)
{
...
void trap_CM_TransformedBoxTrace(trace_t *results, const vec3_t start, const vec3_t end,
								 ...
								 const vec3_t origin, const vec3_t angles, qboolean customEpsilonTrace, float customEpsilon, int traceCustomFlags // coolapi extension. normally these extra values would be random/undefined anyway so we may as well only check engine side
)
{
...
{
	syscall(CG_SETUSERCMDVALUE, flags, cmd, serverTime, angles, buttons, weapon,
			forceSelect, invenSelect, generic_cmd, forward, right, up, PASSFLOAT(sensitivityScale));
}
