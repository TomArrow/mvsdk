...
void MVAPI_AfterInit(void)
{
	...
	if (mvapi >= 4)
		submodelBypass = trap_MVAPI_EnableSubmodelBypass(qtrue);
...
void CG_UpdateCvars(void)
{
	...
	// Register V24 Enhanced Features CVARs
	CG_RegisterV24Cvars();
}
...
void MV_UpdateCgFlags(void)
{
	...
	if (submodelBypass)
		intValue |= MVSDK_CGFLAG_SUBMODEL_BYPASS;
	...
	if (registered && mvsdk_cgFlags.integer == intValue)
		return;
...
void CG_InitAngleVectorsLUT()
{ // for realaccel strafehelper
	int i;
	vec3_t angles = {0, 0, 0};
	if (cg_developer.integer)
	{
...
void CG_Init(int serverMessageNum, int serverCommandSequence, int clientNum)
{
	...
	{ // We don't know the version of the server, yet...
		...
		if (jk2version == VERSION_UNDEF)
			CG_Error("MVSDK: Unable to detect jk2version [CGame].");
	...
	*/
	...
	s = cgs.mapname + 5;
	if (s)
	{ // exec cfg for custom map specific remaps/skies/other stuff
	...
	// Initialize V24 Enhanced Features
	CG_InitESP();
}
