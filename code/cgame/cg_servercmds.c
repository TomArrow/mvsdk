...
static void CG_PauseGameEnded(void)
{
	...
	if (!cg.pausedGame)
		return; // Demo might start inside a pause.
	...
	void CG_UpdateConfigString(int num, qboolean init)
	{
		...
		else
		{
			...
			case CS_INTERMISSION:
				...
				{ // Game just ended. Intermission will come soon. Request scoreboard data NOW before players disconnect/ragequit.
	...
	*/
	...
	static void CG_ServerCommand(void)
	{
		...
		if (!strcmp(cmd, "cp"))
		{
			...
			subtype = CG_Argv(3);
			if ((cg_customizeRace.integer & CUSTOMIZERACE_HIDECHECKPOINTCP) && !Q_stricmp("cptimer", type) || (cg_customizeRace.integer & CUSTOMIZERACE_HIDEROLLSPEEDCP) && !Q_stricmp("rollspeed", type) || (cg_customizeRace.integer & CUSTOMIZERACE_HIDEANTILOOPRESTART) && !Q_stricmp("antiloop", type) && !Q_stricmp("restart", subtype) || (cg_customizeRace.integer & CUSTOMIZERACE_HIDEDEFAULTRACESTART) && !Q_stricmp("racestarted", type) && !Q_stricmp("normal", subtype))
			{
		...
		{
			if (trap_Argc() == 4 && cg_remaps.integer && Q_stricmpn(CG_Argv(1), "console", 7))
			{ // JAPRO - Clientside - Allow noremaps
