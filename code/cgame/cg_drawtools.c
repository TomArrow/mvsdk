...
void CG_FillRect(float x, float y, float width, float height, const float *color)
{
	...
	float sw = width * cgs.screenXFactorInv;
	float sh = height * cgs.screenYFactorInv;
	trap_R_DrawStretchPic(sx, sy, sw, sh, 0, 0, 0, 0, cgs.media.whiteShader);
	trap_R_SetColor(NULL);
}
...
void CG_DrawPic(float x, float y, float width, float height, qhandle_t hShader)
{
	...
	float sw = width * cgs.screenXFactorInv;
	float sh = height * cgs.screenYFactorInv;
	trap_R_DrawStretchPic(sx, sy, sw, sh, 0, 0, 1, 1, hShader);
}
...
void CG_DrawRotatePic(float x, float y, float width, float height, float angle, qhandle_t hShader)
{
	...
	float sw = width * cgs.screenXFactorInv;
	float sh = height * cgs.screenYFactorInv;
	trap_R_DrawRotatePic(sx, sy, sw, sh, 0, 0, 1, 1, angle, hShader);
}
...
void CG_DrawRotatePic2(float x, float y, float width, float height, float angle, qhandle_t hShader)
{
	...
	float sw = width * cgs.screenXFactorInv;
	float sh = height * cgs.screenYFactorInv;
	trap_R_DrawRotatePic2(sx, sy, sw, sh, 0, 0, 1, 1, angle, hShader);
}
