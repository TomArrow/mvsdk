...
void G_CheckIronManStatus() {
	...
	for (i = 0; i < level.maxclients; i++, ent++) {
		if (!ent->inuse || !ent->client || ent->client->pers.connected != CON_CONNECTED || ent->client->sess.mode != MODE_IRONMAN || ent->client->sess.sessionTeam == TEAM_SPECTATOR) {
			if (ent->inuse && ent->client && ent->client->pers.connected >= CON_CONNECTING && ent->client->sess.mode == MODE_IRONMAN && ent->client->sess.modeTeam == MODETEAM_IRONMAN_CAPPER) {
...
void G_RunFrame( int levelTime ) {
	...
	G_BufferedSendOrPrintFlushIfNeeded(NULL, qtrue, qfalse);
